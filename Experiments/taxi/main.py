...
data = pickle.load(open("10_adjmat_full.pkl", "rb"))
(N, l, m, v) = data.shape
print("Shape =", data.shape)

# scaler = StandardScaler()
# data = scaler.fit_transform(data.reshape((-1,1))).reshape((N,l,m,v))
...
def train_ltari(train, test):
    ...
    forecast = invert_diff(diff_forecast, train[-1:])
    return err(test, forecast)
...
def train_lstm(train, test):
    ...
    lstm.add(GRU(1024, activation='relu',return_sequences=True))
    lstm.add(GRU(512, activation="relu"))
    lstm.add(Dense(512))
    ...
    for i in range(n_steps,N_test):
        ...
        forecast[i] = lstm.predict(currX, verbose=0).reshape((l, m, v))
