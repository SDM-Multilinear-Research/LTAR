...
class LMLDS():
    def __init__(self, train, maxiter = 20):
    ...
    def fit(self, transformation = "dct"):
        ...
        for i in range(self.matrix_shape[1]):
            ...
            model = LDS(train_df, maxiter=self.maxiter)
            model.fit()
            fits.append(model) 
        self.lds_fits = fits
    ...
    def single_step_forecast(self, interval, test_tensor):
        matrix_shape = self.matrix_shape
        l_forecast_tensor = np.zeros((interval, matrix_shape[0], matrix_shape[1]))
        l_test_tensor = self.__apply_trans(test_tensor, self.transformation, 2)

        # predict for every lateral slice
        for i in range(self.matrix_shape[1]):
            lat_slice = self.lds_fits[i].single_step_forecast(interval, l_test_tensor[:,:,i])
            l_forecast_tensor[:,:,i] = lat_slice
